...
from matcha.pointmap.depthanythingv2 import depth_linear_align_ransac
...
if __name__ == "__main__":
    ...
    save_root_path = os.path.dirname(args.data_path)
    vis_dir = os.path.join(save_root_path, 'vis_linear_align_use_depth_ransac')
    os.makedirs(vis_dir, exist_ok=True)
    cat_dir = os.path.join(save_root_path, 'cat_vis_linear_align_use_depth_ransac')
    ...
    for i in range(len(train_viewpoints)):
        ...
        for plane_id in plane_id_list:
            ...
            mono_depth = torch.from_numpy(mono_depth).to('cuda')

            # get valid depth for alignment
            aligned_depth, alpha, beta, inlier_ratio = depth_linear_align_ransac(mono_depth, depth, valid_mask, return_alpha_beta=True)

            if vis_mask:
                # draw scatter figure for mono_depth and depth
                mono_depth_valid = mono_depth[valid_mask].cpu().numpy()
                depth_valid = depth[valid_mask].cpu().numpy()

                plt.figure(figsize=(10, 6))
                
                # Scatter plot of mono_disp vs true_disp
                plt.scatter(mono_depth_valid, depth_valid, alpha=0.6, s=1, c='blue')
                
                # Plot the fitted line: true_disp = beta * mono_disp + alpha
                mono_range = np.linspace(mono_depth_valid.min(), mono_depth_valid.max(), 100)
                ...
                plt.title(f'Depth Alignment (Plane {plane_id}, Frame {i}, {valid_points_num} valid points)')
                plt.legend()
                plt.grid(True, alpha=0.3)

                # Save the analysis plot
                analysis_save_path = os.path.join(vis_dir, f'analysis_frame{i:06d}_plane{plane_id:06d}_inlier_ratio{inlier_ratio:.2f}.png')
                ...
                cat_img.save(os.path.join(cat_dir, f'frame{i:06d}_plane{plane_id:06d}_inlier_ratio{inlier_ratio:.2f}.png'))
