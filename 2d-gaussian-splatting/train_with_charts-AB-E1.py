...
def training(
    ...
    dense_regul, use_relative_depth_reg
):
    ...
    if dense_data_path is not None:
        ...
        if len(see3d_gs_cameras_list) > 0:
            use_dense_supervision = True            # use see3d gs as dense supervision
            dense_viewpoint_cams = see3d_gs_cameras_list
            dense_viewpoint_idx_stack = None
            use_chart_view_every_n_iter = 3
        else:
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if iteration % use_chart_view_every_n_iter == 0:
            ...
            if not use_relative_depth_reg:                                  # hard code for relative depth regularization
                depth_prior_loss = torch.zeros_like(loss.detach())
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_relative_depth_reg', action='store_true', default=False)
