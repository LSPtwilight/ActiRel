...
if __name__ == '__main__':
    ...
    for i, view_id in enumerate(see3d_view_id):
        ...
        dst_img_path = os.path.join(inpaint_path, f'predict_warp_frame{i:06d}.png')
        shutil.copy(src_img_path, dst_img_path)
        print(f'predict_warp_frame{i:06d}.png copied')
    print(f'{len(see3d_view_id)} images copied')

    # save pseudo see3d poses
    save_cameras = {}
    save_cameras['train_views'] = len(chart_view_id)
    cam_dict = {vp.image_name.replace('.png', ''): vp for vp in all_viewpoints}

    for idx, view_id in enumerate(see3d_view_id):
        key = f'{view_id:06d}'
        see3d_viewpoint = cam_dict[key]          
        assert see3d_viewpoint.image_name.replace('.png', '') == key

        save_cameras[f'R_{idx:06d}'] = see3d_viewpoint.R
        save_cameras[f'T_{idx:06d}'] = see3d_viewpoint.T
        save_cameras[f'FoVx_{idx:06d}'] = see3d_viewpoint.FoVx
        save_cameras[f'FoVy_{idx:06d}'] = see3d_viewpoint.FoVy
        save_cameras[f'image_width_{idx:06d}'] = see3d_viewpoint.image_width
        save_cameras[f'image_height_{idx:06d}'] = see3d_viewpoint.image_height

    # save need inpaint views cameras
    save_cameras['n_views'] = len(see3d_view_id)
