...
def training_report(output_path, tb_writer, iteration, Ll1, loss, dist_loss, normal_loss, elapsed, testing_iterations, 
                    gaussians, test_cameras, renderFunc, renderArgs):
    if tb_writer:
        tb_writer.add_scalar('train_loss_patches/reg_loss', Ll1.item(), iteration)
        tb_writer.add_scalar('train_loss_patches/total_loss', loss.item(), iteration)
        tb_writer.add_scalar('iter_time', elapsed, iteration)
        tb_writer.add_scalar('total_points', gaussians.get_xyz.shape[0], iteration)  # Only count new gaussians
...
if __name__ == "__main__":
    ...
    total_pcds = torch.cat([charts_pts, none_visible_pcds], dim=0)
    total_pcds_colors = torch.cat([charts_pts_colors, none_visible_pcd_colors], dim=0)
    save_tensor_as_pcd(total_pcds, os.path.join(output_path, 'total_pcds.ply'), total_pcds_colors)

    if args.use_downsample:
        total_pcds, total_pcds_colors = downsample_pcd(total_pcds, total_pcds_colors, method='voxel')
        save_tensor_as_pcd(total_pcds, os.path.join(output_path, 'total_pcds_downsampled.ply'), total_pcds_colors)

    # Initialize new Gaussians
    init_pcd = BasicPointCloud(points=total_pcds.cpu().numpy(), colors=total_pcds_colors.cpu().numpy(), normals=None)
    gaussians.create_from_pcd(init_pcd, gaussians.spatial_lr_scale)
    gaussians.set_mip_filter(True)
    gaussians.compute_mip_filter(cameras=see3d_pointmap_cameras.gs_cameras)
