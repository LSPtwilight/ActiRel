...
def training(
    ...
    charts_points = depths_to_points_parallel(charts_depths, viewpoint_cams)
    print("[INFO] Charts priors built.")

    if use_visibility_mask:
        print("[INFO] Building visibility masks...")
        charts_visibility_masks = build_visibility_masks(viewpoint_cams, charts_depths, charts_points) # list of [1, H, W]

        # vis visibility masks
        train_view_num = len(viewpoint_cams)
        for idx in range(train_view_num):
            vis_map = charts_visibility_masks[idx].squeeze(0).cpu().numpy()
            vis_map = Image.fromarray((vis_map * 255).astype('uint8'))
            save_root_path = f'{dataset.model_path}/see3d_render/vis_masks'
            os.makedirs(save_root_path, exist_ok=True)
            save_path = os.path.join(save_root_path, f'vis_mask_{idx}.png')
            vis_map.save(save_path)
        
        print("[INFO] Visibility masks built.")
    ...
    if create_gaussians_from_charts_data:    
        ...
        _visibility_masks = [
            torch.nn.functional.interpolate(mask[None].cuda(), (h_charts, w_charts), mode="nearest")[0]
            for mask in charts_visibility_masks
        ]
        gaussian_params = get_gaussian_parameters_from_charts_data(
            ...
            visibility_masks=_visibility_masks,
        )
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if use_visibility_mask:
            visibility_mask = charts_visibility_masks[viewpoint_idx]                # [1, H, W]
        else:
            visibility_mask = torch.ones_like(charts_confs[viewpoint_idx])          # [1, H, W]
        ...
        if iteration % use_chart_view_every_n_iter == 0:
            ...
            if False:  # New (should be used with depth_ratio = 1.)
                ...
                # Depth regularization
                depth_prior_loss = lambda_prior_depth * (
                    confidence_weighting * charts_confs[viewpoint_idx] * visibility_mask *  # (1, h, w)
            ...
            normal_prior_loss = lambda_prior_normal * ((1. - (rend_normal * charts_normals[viewpoint_idx]).sum(dim=0)) * visibility_mask.squeeze(0)).mean()
            
            # Curvature regularization
            curv_prior_loss = lambda_prior_curvature * ((charts_curvs[viewpoint_idx] - rend_curvature).abs() * visibility_mask).mean()
