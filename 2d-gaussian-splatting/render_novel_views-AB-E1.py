...
if __name__ == "__main__":
    ...
    novel_poses_1, novel_cams_1 = generate_see3d_camera_by_lookat_object_centric(train_viewpoints, visibility_grid, n_frames=30)
    novel_poses.extend(novel_poses_1)
    novel_cams.extend(novel_cams_1)

    # look at scene around
    novel_poses_2, novel_cams_2 = generate_see3d_camera_by_lookat(input_viewpoints, visibility_grid, gs_input_view_depths.squeeze(1), gs_input_view_points, n_frames=30)
    novel_poses.extend(novel_poses_2)
    novel_cams.extend(novel_cams_2)

    novel_poses_3, novel_cams_3 = generate_see3d_camera_by_view_angle(input_viewpoints, visibility_grid, fovy_deg=80, n_frames=30)
    ...
    hard_code_select_num = 20
    # need_inpaint_views = select_need_inpaint_views(novel_cams, gs_none_visible_rate, gaussians, hard_code_select_num, none_visible_rate_high_bound=0.5, covisible_rate_high_bound=0.9)
    
    # random select
    need_inpaint_views = np.random.choice(len(novel_cams), hard_code_select_num, replace=False)
