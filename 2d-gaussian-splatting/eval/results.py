...
def get_merge_results(exp_root_path, exp_key):
    ...
    for exp_name in exp_list:
        if exp_key in exp_name:
            ...
            exp_results_path = os.path.join(exp_root_path, exp_name, 'result.json')
...
if __name__ == '__main__':
    ...
    for scan_id in range(1, scene_num+1):
        ...
        merge_results = get_merge_results(exp_root_path, exp_key)
        ...
        save_json_path = os.path.join(exp_root_path, f'scan{scan_id}_{exp_key}_merge_results.json')
    ...
    save_json_path = os.path.join(root_path, f'average_{exp_key}_merge_results.json')
    if os.path.exists(save_json_path):
        # print(f'{save_json_path} exists, remove it')
        os.remove(save_json_path)
    with open(save_json_path, 'w') as f:
    ...
    save_csv_path = os.path.join(root_path, f'average_{exp_key}_merge_results.csv')
