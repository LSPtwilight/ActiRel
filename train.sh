...
do
    ...
    do
        ...
        select_inpaint_num=20
        echo "Training scan$i-$test_str with $config_view_num views"
        python train.py -s data/replica/scan$i/ -o output/replica-$config_view_num-views/scan$i-$test_str-scratch/ --sfm_config posed --use_view_config --config_view_num $config_view_num --select_inpaint_num $select_inpaint_num --scratch_train
        echo "Finished training scan$i-$test_str with $config_view_num views"
    done
done
echo "Finished training all scans"
